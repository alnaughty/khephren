...
class SalesContract extends _i1.GeneratedContract {
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<TotalContributions> totalContributions(_i1.EthereumAddress account,
      {_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[23];
    assert(checkSignature(function, '7ed3deba'));
    final params = [account];
    final response = await read(function, params, atBlock);
    return TotalContributions(response);
  }
...
class TotalContributions {
  TotalContributions(List<dynamic> response)
      : var1 = (response[0] as BigInt),
        var2 = (response[1] as BigInt);
