...
class Pool extends _i1.GeneratedContract {
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> addTokenRewards(
      BigInt _amount, _i1.EthereumAddress _rewardAddress,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[1];
    assert(checkSignature(function, '6934b9ff'));
    final params = [_amount, _rewardAddress];
    return write(credentials, transaction, function, params);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> poolIndex({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[6];
    assert(checkSignature(function, '4ce272ee'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
